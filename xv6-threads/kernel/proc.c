...
#define KTHREAD_DBG(fmt, ...) printf("[KTHREAD-KERNEL] " fmt "\n", ##__VA_ARGS__)
...
// Kernel thread API implementation
int kthread_create(void (*start_func)(), void *stack, uint64 stack_size) {
    ...
    // Copy parent's user memory and trapframe
    ...
    *(nt->trapframe) = *(p->trapframe);
    ...
    // Set up the trapframe for the new thread
    nt->trapframe->epc = (uint64)start_func; // entry point
    nt->trapframe->sp = (uint64)stack + stack_size; // top of stack
    // For argument passing, we set a0 to the stack address (convention for RISC-V)
    // The thread can then access its argument at the bottom of its stack
...
void kthread_exit(int status) {
  ...
  // We shouldn't close file descriptors as they're shared with the process
  ...
  // Wake any thread or process waiting on this thread
  // Use the tid directly as a channel for better wakeup targeting
...
int kthread_join(int tid, int *status) {
  ...
  // First check if thread has already been joined
  ...
  for(;;) {
    ...
    // After waking up, we'll loop again to check the thread's status
  }
}
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        /* kthread_dbg("scheduler: running pid=%d\n", p->pid); */ // DISABLED: too verbose, causes lag
